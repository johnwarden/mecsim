...
- if the final allocations are close to the overall optimal (maximum utility for the group)
- if results disproportionately benefit some individuals at the expense of others?
- if the final reports are incentive-aligned (individuals reports are to their "true optimal" allocation)?
...
Users then take turns strategically updating their reports to maximize their own utility given the reports of other users.

The simulation terminates if no user is able to improve their utility more than some threshhold. 

## Results and Observations

The most striking result of these experiments is how close to *optimal* the results are. The total utility is very close to the maximum possible total utility for most preference profiles. I didn't expect this when I created this simulator.

The following is the output of the simulation for all combinations of mechanisms and preferences currently implemented. CAVEAT: since these are averages across preference profiles, they tell us as much about the preference profiles that have been implemented as they do about the mechansim. However, I have implemented a variety of preference profiles. So the interesting result is how close to optimality many mechanisms are across many preference profiles.

    ┌─────────────────────┬─────────────┬─────────────────┬──────────────┬─────────────────────┬─────────────────────────┐
    │ Mechanism           │ Mean Rounds │ Equilibrium (%) │ Mean Utility │ Mean Optimality (%) │ Mean Incent. Align. (%) │
    ├─────────────────────┼─────────────┼─────────────────┼──────────────┼─────────────────────┼─────────────────────────┤
    │ Mean                │        3.27 │           100.0 │         0.82 │                90.4 │                    80.8 │
    │ PairwiseProbability │        1.36 │           100.0 │        0.777 │                85.9 │                    96.2 │
    │ QuadraticFunding    │        3.91 │            90.9 │        0.818 │                90.2 │                    75.4 │
    │ SAP                 │        1.73 │           100.0 │        0.849 │                93.2 │                    80.8 │
    │ SAPScaled           │        6.82 │            45.5 │        0.798 │                87.6 │                    78.8 │
    └─────────────────────┴─────────────┴─────────────────┴──────────────┴─────────────────────┴─────────────────────────┘
